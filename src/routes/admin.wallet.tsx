...
function WalletPage() {
  ...
  const submit = async () => {
    ...
    if (!result.ok) {
      ...
      navigate({
        to: "/admin/transaction-failed/$ref",
        params: { ref: result.failure.ref },
      });
      return;
    }
    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Fund the disbursement wallet. Transfers only fire from this balance.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_360px]">
        ...
        >
          ...
          <div className="mt-1 text-xs text-muted-foreground">
            {compactNaira(balance)} · Squad sandbox
          </div>
          ...
          <div className="mt-6 space-y-3">
            ...
            <p className="text-[11px] text-muted-foreground">
              ~15% of mock fundings will fail and route to a help page so you can see the failure
              flow.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-[var(--shadow-card)]">
          ...
          {failures.length === 0 ? (
            <p className="mt-3 text-xs text-muted-foreground">
              No failed transactions. The Squad gateway is happy.
            </p>
          ) : (
