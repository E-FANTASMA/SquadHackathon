...
function ClaimPage() {
  ...
  const matched = user?.matchedEmployeeId
    ? useLedgerStore.getState().employees.find((e) => e.id === user.matchedEmployeeId)
    : undefined;
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-6 sm:max-w-xl sm:py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Match your NIN and 10-digit NUBAN against your employer's payroll. This is the first
        verification check.
      </p>
      ...
      <div className="mt-6 rounded-xl border bg-card p-5 shadow-[var(--shadow-card)]">
        <div className="space-y-3">
          ...
          <Button className="h-12 w-full text-base" disabled={busy} onClick={() => void onSearch()}>
            <Search className="h-4 w-4" /> {busy ? "Searching…" : "Search payroll"}
          </Button>
          <p className="text-[11px] text-muted-foreground">
            Demo tip: try NIN <span className="font-mono">11111111111</span> or use the account on a
            worker shown in the admin ledger.
          </p>
        </div>
      </div>
      ...
      {result === "match" && matched && (
        <div
          className="mt-6 rounded-xl border bg-card p-5 shadow-[var(--shadow-card)]"
          style={{
            background: "color-mix(in oklab, var(--status-verified) 10%, transparent)",
            borderColor: "color-mix(in oklab, var(--status-verified) 35%, transparent)",
          }}
        >
      ...
      {result === "miss" && (
        <div
          className="mt-6 rounded-xl border bg-card p-5 shadow-[var(--shadow-card)]"
          style={{
            background: "color-mix(in oklab, var(--status-rejected) 10%, transparent)",
            borderColor: "color-mix(in oklab, var(--status-rejected) 35%, transparent)",
          }}
        >
          ...
          <p className="mt-2 text-sm">
            Your NIN and account combination doesn't match any record in your employer's payroll.
            This can happen if your details were updated (e.g. name change) or the wrong account was
            registered.
          </p>
          <Button asChild className="mt-4 w-full">
