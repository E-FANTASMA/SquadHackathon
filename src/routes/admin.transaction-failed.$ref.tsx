      ...
      <div className="rounded-2xl border bg-card p-8 shadow-[var(--shadow-card)]">
        <div className="flex items-center gap-3">
          <span
            ...
            style={{
              background: "color-mix(in oklab, var(--status-rejected) 14%, transparent)",
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Squad rejected the request. Funds were not moved.
            </p>
          </div>
        </div>
          ...
          <>
            <dl className="mt-6 grid gap-3 rounded-lg border bg-background p-4 text-sm sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                ...
                <div className="mt-1 rounded-md border bg-muted/30 p-3 font-mono text-xs">
                  {failure.message}
                </div>
              </div>
            </dl>
            ...
            <ol className="mt-3 space-y-2">
              {(
                FIXES[failure.code] ?? [
                  "Retry the transaction. If it fails again, contact Squad support with the reference above.",
                ]
              ).map((tip, i) => (
        ...
        <div className="mt-8 flex flex-wrap gap-2">
          ...
          <Button asChild>
            <Link to="/admin/wallet">
              <RefreshCcw className="h-4 w-4" /> Retry funding
            </Link>
          </Button>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{
          color: tone === "danger" ? "var(--status-rejected)" : undefined,
        }}
      >
        {value}
      </div>
