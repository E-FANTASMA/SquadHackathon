...
export const Route = createFileRoute("/audit")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Printable audit trail and compliance report for government clients.",
      },
      ...
      <article className="rounded-lg border bg-card p-8 shadow-[var(--shadow-card)]">
        <header className="border-b pb-6">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Generated {new Date().toLocaleString()} · Audit firm reference: PG-
              ...
              <tbody>
                {employees.slice(0, 40).map((e) => (
                  <tr key={e.id} className="border-b">
                    ...
                    <td className="py-2">
                      <SquadPaymentGate status={e.squadStatus} />
                    </td>
                  </tr>
                ))}
              </tbody>
