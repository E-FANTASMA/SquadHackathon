      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-3 text-sm" style={{ color: meta.color }}>
              {meta.description}
            </p>
          </div>
          {employee && (
            <div className="text-right">
              ...
              <div className="text-3xl font-semibold tabular-nums" style={{ color: meta.color }}>
                {employee.trustScore}
                <span className="text-base opacity-70">/{MAX_SCORE}</span>
              </div>
            </div>
          )}
        </div>
      ...
      {employee && (
        <section className="mt-6 rounded-xl border bg-card p-5 shadow-[var(--shadow-card)]">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Each check is awarded by the AI from your documents.
          </p>
          <div className="mt-4">
...
function ActionCard({
  ...
  return (
    <Link
      to={href}
      className="block rounded-xl border bg-card p-5 shadow-[var(--shadow-card)] transition-shadow hover:shadow-[var(--shadow-elegant)]"
    >
      <div className="flex items-start gap-3">
        <span
          ...
          style={{
            background:
              tone === "danger"
                ? "color-mix(in oklab, var(--status-rejected) 14%, transparent)"
