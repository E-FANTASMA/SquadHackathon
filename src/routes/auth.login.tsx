...
function LoginPage() {
  ...
  const handle = (role: "worker" | "company_admin") => {
    ...
    console.log("LoginPage - login attempt:", { email, role });
    login(email, role);
    const user = useAuthStore.getState().user;
    console.log("LoginPage - user after login:", user);
      ...
      <div className="w-full rounded-2xl border bg-card p-6 shadow-[var(--shadow-card)]">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-6">
          ...
          <TabsContent value="worker" className="mt-4">
            <Button className="w-full" onClick={() => handle("worker")}>
              Continue to Worker Portal
            </Button>
          </TabsContent>
          <TabsContent value="company" className="mt-4">
            <Button className="w-full" onClick={() => handle("company_admin")}>
