...
function AppealPage() {
  ...
  const onSubmit = () => {
    if (message.trim().length < 20)
      return toast.error("Please describe the issue (min 20 characters)");
    ...
    pushFeed({
      kind: "info",
      message: `Appeal filed by ${user?.fullName ?? "worker"}`,
    });
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-6 sm:max-w-xl sm:py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Tell your auditor what happened and attach one supporting document (e.g. National ID,
        marriage certificate, NIN slip).
      </p>
      ...
      <div className="mt-6 space-y-4 rounded-xl border bg-card p-5 shadow-[var(--shadow-card)]">
        <div className="space-y-1.5">
          ...
          <Textarea
            ...
            placeholder="My NIN was updated after marriage, the payroll still has my old name. Attaching my new National ID for verification."
          />
        ...
        <div className="space-y-1.5">
          ...
          >
            ...
            <span className="flex-1 truncate">
              {doc ? doc.name : "Tap to attach National ID, NIN slip, or other proof"}
            </span>
