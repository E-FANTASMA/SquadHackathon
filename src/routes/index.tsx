  ...
  head: () => ({
    meta: [
      { title: "PayGuard AI — Smart Payroll Trust" },
      {
        name: "description",
        content:
          "Dual-portal payroll trust system. AI-gated Squad disbursement for Nigerian companies and verified workers.",
      },
      { property: "og:title", content: "PayGuard AI" },
      {
        property: "og:description",
        content: "Stop payroll fraud before money leaves the bank.",
      },
    ],
  }),
  component: Landing,
    ...
    <>
      ...
      >
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-16 md:py-24">
          ...
          <p className="mt-5 max-w-2xl text-base text-white/80 md:text-lg">
            PayGuard AI is a dual-portal trust system. Companies upload payroll. Workers claim their
            record and submit proof. The AI scores every payment 0&ndash;110 and Squad only releases
            funds for verified records.
          </p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          Two distinct experiences, one shared trust engine. Auditors get a desktop command center;
          workers get a mobile-first claim flow.
        </p>
        <div className="mt-8 grid gap-5 md:grid-cols-2">
          <PortalCard
            ...
            features={[
              "Payroll batch manager",
              "Wallet funding & disbursement",
              "Smart Decision Ledger",
              "Appeals inbox",
            ]}
          ...
          <PortalCard
            ...
            features={[
              "NIN / Account claim wizard",
              "Document upload (PDF + image)",
              "Live verification status",
              "Appeal a rejected score",
            ]}
            cta="Sign up as a Worker"
            to="/auth/signup"
            search={{ role: "worker" }}
            secondary={{ label: "Demo worker home", to: "/worker/home" }}
          />
        </div>
      </section>
      ...
      <section className="bg-[var(--gradient-trust)] py-16">
        <div className="mx-auto max-w-7xl px-4">
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Every worker is graded on six checks. The total score routes them into one of three
            buckets, and Squad is locked accordingly.
          </p>
          <div className="mt-8 grid gap-3 md:grid-cols-3">
            <Bucket
              ...
              range="80&ndash;110"
              label="Verified"
              desc="Squad transfer enabled. Money moves."
            />
            <Bucket
              color="var(--status-flagged)"
              range="50&ndash;79"
              label="Flagged"
              desc="Manual review. Admin can override after reading evidence."
            />
            <Bucket
              color="var(--status-rejected)"
              range="Below 50"
              label="Rejected"
              desc="Squad locked. Worker must request an appeal."
            />
          </div>
                ...
                {[
                  {
                    ...
                    body: "Mobile-first worker UI works on low-end Android phones over slow networks.",
                  },
                  {
                    icon: <Lock className="h-4 w-4" />,
                    body: "Squad transfers cannot fire for any worker below score 80, full stop.",
                  },
                  {
                    icon: <Wallet className="h-4 w-4" />,
                    body: "Failed wallet funding redirects to a dedicated help page with the Squad reference and fix.",
                  },
                  {
                    icon: <ShieldCheck className="h-4 w-4" />,
                    body: "Auditor overrides and appeal approvals are recorded in the audit trail.",
                  },
                ].map((b, i) => (
...
function PortalCard({
  ...
  search?: any;
  secondary: { label: string; to: FileRouteTypes["to"] };
}) {
  return (
    <div className="group relative overflow-hidden rounded-2xl border bg-card p-6 shadow-[var(--shadow-card)] transition-shadow hover:shadow-[var(--shadow-elegant)]">
      <div
        ...
        style={{
          background: tone === "primary" ? "var(--gradient-primary)" : "var(--status-verified)",
        }}
      />
