...
export function LedgerTable() {
  ...
  const filtered = useMemo(() => {
    return employees.filter((e) => {
      if (
        search &&
        !`${e.name} ${e.id} ${e.account} ${e.nin}`.toLowerCase().includes(search.toLowerCase())
      )
        return false;
  ...
  const columns = useMemo<ColumnDef<Employee>[]>(
    () => [
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 text-sm font-medium">
              ...
              {row.original.checks.ninVerified && (
                <ShieldCheck
                  className="h-3.5 w-3.5 text-[color:var(--status-verified)]"
                  aria-label="NIN verified"
                />
              )}
            </div>
      ...
      },
      {
        ...
        header: "Salary",
        cell: ({ row }) => (
          <span className="tabular-nums text-sm">{naira(row.original.salary)}</span>
        ),
      },
      {
  ...
  const verifiedCount = employees.filter(
    (e) => e.verificationStatus === "verified" || e.override,
  ).length;
  const flaggedCount = employees.filter(
    (e) => e.verificationStatus === "flagged" && !e.override,
  ).length;
  const rejectedCount = employees.filter(
    (e) => e.verificationStatus === "rejected" && !e.override,
  ).length;
  const blockedSavings = employees
    .filter(
      (e) =>
        (e.verificationStatus === "flagged" || e.verificationStatus === "rejected") && !e.override,
    )
  ...
  return (
    <div className="rounded-lg border bg-card shadow-[var(--shadow-card)]">
            ...
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id}>
                {hg.headers.map((h) => {
                  const sortable =
                    h.column.getCanSort() &&
                    !["expander", "override", "action"].includes(h.column.id);
                  return (
      ...
      <div className="grid gap-3 border-t bg-[var(--gradient-trust)] p-4 sm:grid-cols-4">
        ...
        <Summary
          label="Rejected (Savings)"
          value={naira(blockedSavings)}
          tone="danger"
          sub={`${rejectedCount} records`}
        />
      </div>
          ...
          {reviewing && (
            <ManualReview
              ...
              onSave={(checks) => {
                ...
                pushFeed({
                  kind: "override",
                  message: `Manual review · ${reviewing.id} re-scored`,
                });
...
function ManualReview({
  ...
  employee: Employee;
  onSave: (c: VerificationChecks) => void;
}) {
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          {employee.id} · {employee.department} · Tap any check to award or remove its points.
      ...
      <DialogFooter>
        <Button variant="outline" onClick={() => setChecks(employee.checks)}>
          Reset
        </Button>
  ...
  return (
    <div className="flex items-center gap-2">
      <div
        ...
        style={{
          background: `conic-gradient(${color} ${pct}deg, color-mix(in oklab, ${color} 18%, transparent) 0)`,
        }}
      >
        <span
          className="grid h-7 w-7 place-items-center rounded-full text-foreground"
